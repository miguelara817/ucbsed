  ...
  <div class="container-xl">
      @if ($procesos)
        <div class="row row-deck row-cards">
          <div class="col-12">
            <div class="card">
              <div class="card-body">
                <form action="{{ route('reporteseval.seleccion') }}" method="post">
                  @csrf
                  <div class="form-group mb-3">
                    <h3 class="card-title">Seleccione un proceso</h3>
                    <div class="d-flex flex-lg-row align-center">
                      <div style="width: 100%; margin-right:10px">
                          <select name="evalprocess_id" id="version" class='form-control' required>
                              <option value="" selected="selected" disabled>Selecciona una opción</option>
                         
                              @foreach ($evalprocess_all as $process) 
                                  <option value="{{$process->id}}">Proceso: {{$process->id}} | Periodo: {{date('d/m/Y',strtotime($process->fecha_inicio))}} a {{date('d/m/Y',strtotime($process->fecha_conclusion))}}</option>
                              @endforeach
                          </select>
                      </div>
                      
                      <div class="btn-list">
                        <input type="submit" class="btn btn-primary ms-auto ajax-submit" value="Generar reporte">
                      </div>
                    </div>
                  </div> 
                </form>
      ...
      <div class="row row-deck row-cards">
        <div class="col-12">
          <div class="card">
            <div class="card-body">
              <h3>No se econtraron procesos de evaluación</h3>
            </div>
          </div>
        </div>
      </div>
